// NOTE: The following tests are taken from the consensys token repository
...
contract('ERC20', (accounts) => {
  beforeEach(async () => {
    HST = await EIP20Abstraction.new(
      args.name,
      args.symbol,
      args.decimals,
      args.totalSupply,
  ...
  it('creation: should succeed in creating over 2^256 - 1 (max) tokens', async () => {
    // 2^256 - 1
    const maxTotalSupply = '115792089237316195423570985008687907853269984665640564039457584007913129639935'
    ...
    let totalSupply = await HST2.totalSupply();
    totalSupply = totalSupply.toString();
    assert.strictEqual(totalSupply, maxTotalSupply);
  ...
  it('approvals: approve max (2^256 - 1)', async () => {
    ...
    await HST.approve(accounts[1], maxAllowance, { from: accounts[0] });
    let allowance = await HST.allowance(accounts[0], accounts[1]);
    allowance = allowance.toString();
    assert.strictEqual(allowance, maxAllowance);
  ...
  it('approvals: msg.sender approves accounts[1] of max (2^256 - 1) & withdraws 20', async () => {
    ...
    await HST.transferFrom(accounts[0], accounts[2], 20, { from: accounts[1] });
    let allowance01 = await HST.allowance.call(accounts[0], accounts[1]);
    //allowance01 = allowance01.toNumber();
    assert.strictEqual(allowance01.toString(), '115792089237316195423570985008687907853269984665640564039457584007913129639915');
